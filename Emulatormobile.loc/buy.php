...
<div style=" background-color: darkblue;color: white ;  margin-right: 80%; padding-bottom: 100px">
    <h1>Smart Basket <a  style="color: white; font-size: 50%;  " href= "en/index.php">     EN</a></h1>
...
foreach ($result as $buy){
    if ($buy->id_user = $id_user){
        ...
        if ($ids != $id&&$ids !=''){
            echo <<<BOOK
        <p> Адреса магазину <strong>{$address}</strong></p>
        <p>Сума<strong>{$sum}</strong></p>
        <hr>
           
        <p> Покупка № <strong>{$id}</strong></p>
          <p>Товари</p>
...
BOOK;
        ...
        $id_shop= $buy->id_shop;
        if ($ids==''){
            echo "<p> Покупка № <strong>{$id}</strong></p>";
        ...
        <p>{$product_name} <strong>Ціна{$buy->price}</strong></p>
